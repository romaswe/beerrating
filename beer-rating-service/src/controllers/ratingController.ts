...
export const addRating = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const existingRating = await Rating.findOne({ beer: beerId, user: req.user?.id });
        if (existingRating) {
            return res.status(400).json({ message: 'You have already rated this beer.' });
        }
        ...
        const averageRating = allRatings.reduce((acc, r) => acc + r.score, 0) / allRatings.length;
